...
def exercise_list(request):
    ...
    if request.method == 'GET':
        ...
        return Response([serializer.to_representation(e) for e in exercises])

    elif request.method == 'POST':
        serializer = ExerciseSerializer(data=request.data)
        if serializer.is_valid():
            exercise = serializer.save()
            return Response(serializer.to_representation(exercise), status=status.HTTP_201_CREATED)
...
def exercise_detail(request, exercise_id):
    ...
    except (Exercise.DoesNotExist, InvalidId):
        return Response({'error': 'Exercise not found'}, status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = ExerciseSerializer(exercise)
        return Response(serializer.to_representation(exercise))
...
def routine_list(request):
    ...
    if request.method == 'GET':
        ...
        return Response([serializer.to_representation(r) for r in routines])

    elif request.method == 'POST':
        serializer = RoutineSerializer(data=request.data)
        if serializer.is_valid():
            routine = serializer.save()
            return Response(serializer.to_representation(routine), status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def routine_detail(request, routine_id):
    ...
    except (Routine.DoesNotExist, InvalidId):
        return Response({'error': 'Routine not found'}, status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = RoutineSerializer(routine)
        return Response(serializer.to_representation(routine))
...
def routine_adopt(request, routine_id):
    ...
    return Response(serializer.to_representation(new_routine), status=status.HTTP_201_CREATED)
...
def progress_list(request):
    ...
    if request.method == 'GET':
        ...
        return Response([serializer.to_representation(p) for p in progress_entries])

    elif request.method == 'POST':
        serializer = ProgressSerializer(data=request.data)
        if serializer.is_valid():
            progress = serializer.save()
            return Response(serializer.to_representation(progress), status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def progress_detail(request, progress_id):
    ...
    except (Progress.DoesNotExist, InvalidId):
        return Response({'error': 'Progress not found'}, status=status.HTTP_404_NOT_FOUND)
    
    if request.method == 'GET':
        serializer = ProgressSerializer(progress)
        return Response(serializer.to_representation(progress))
...
def recommendation_list(request):
    ...
    if request.method == 'GET':
        ...
        return Response([serializer.to_representation(r) for r in recommendations])

    elif request.method == 'POST':
        serializer = RecommendationSerializer(data=request.data)
        if serializer.is_valid():
            recommendation = serializer.save()
            return Response(serializer.to_representation(recommendation), status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def recommendation_detail(request, recommendation_id):
    ...
    except (Recommendation.DoesNotExist, InvalidId):
        return Response({'error': 'Recommendation not found'}, status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = RecommendationSerializer(recommendation)
        return Response(serializer.to_representation(recommendation))
...
def followup_list(request):
    ...
    if request.method == 'GET':
        ...
        return Response([serializer.to_representation(f) for f in followups])

    elif request.method == 'POST':
        serializer = FollowUpSerializer(data=request.data)
        if serializer.is_valid():
            followup = serializer.save()
            return Response(serializer.to_representation(followup), status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
